document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleNo = async () => {
        ...
        try {
            ...
            elements.geminiInteractionList.innerHTML = `<p class="text-center text-red-500">Não foi possível carregar a explicação. Por favor, tente novamente.</p>`;
    ...
    const getGeminiExplanation = async (word) => {
        const apiKey = "AIzaSyAKpm4N8yVqHwjhwdP9AKMJ9U1s2P3cKA8"; 
        const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${apiKey}`;
        
        const prompt = `Como um tutor de inglês amigável, explique a palavra "${word}" para um aprendiz de nível intermédio. A sua resposta deve ser em português. Siga EXATAMENTE esta estrutura de formatação, usando os títulos entre asteriscos e separando cada secção com "---":
*Introdução da Palavra*
[Uma introdução curta e amigável sobre a palavra.]
---
*Definição Simples*
[Uma definição clara e concisa em uma frase.]
---
*Dica para Memorizar*
[Uma dica mnemónica ou uma associação divertida para ajudar a lembrar.]
---
*Exemplos de Uso*
- [Frase de exemplo em INGLÊS] (Tradução da frase em português)
- [Outra frase de exemplo em INGLÊS] (Tradução da frase em português)
- [Mais uma frase de exemplo em INGLÊS] (Tradução da frase em português)`;
    ...
    const parseAndDisplayGeminiInteraction = (text, word) => {
        elements.geminiInteractionLoading.classList.add('hidden');
        elements.geminiInteractionContent.classList.remove('hidden');
        elements.geminiInteractionWord.textContent = word;
    ...
    const showGeminiDetail = (title) => {
        ...
        if (content.match(exampleRegex)) {
            ...
            elements.geminiDetailContent.innerHTML = `<p>${content}</p>`;
        }
    ...
    const openCelebrationModal = (modal) => {
        if (!modal || isModalTransitioning) return;
        isModalTransitioning = true;
        activeModal = modal;
        modal.classList.add('active');
    ...
    const showLevelUpModal = (levelName, iconSVG) => {
        if (activeModal) {
    ...
    const showAchievementModal = (text, iconSVG) => {
        if (activeModal) {
